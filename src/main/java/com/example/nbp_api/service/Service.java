...
public class Service {
    ...
    static public String MinMax(String code, int n){
        ...
        JSONArray rates = new JSONObject(NbpApi.GetFromURl("http://api.nbp.pl/api/exchangerates/rates/A/"+code+"/last/"+n)).getJSONArray("rates");
    ...
    static public String DifferenceBuyAsk(String code, int n){
        ...
        JSONArray rates = new JSONObject(NbpApi.GetFromURl("http://api.nbp.pl/api/exchangerates/rates/C/"+code+"/last/"+n)).getJSONArray("rates");

        double dif = 0, maxDif = abs(Double.parseDouble(rates.getJSONObject(0).get("bid").toString())-Double.parseDouble(rates.getJSONObject(0).get("ask").toString()));
